...
=end    
    if trainerBattle?
        if level == pkmn.level
          levelAdjust = 1
        elsif level < pkmn.level  #<--- level above foe
            if (level + 3) == pkmn.level
            levelAdjust = 0.8
            elsif (level + 4) == pkmn.level
            levelAdjust = 0.6
            elsif (level + 5) == pkmn.level
            levelAdjust = 0.4
            elsif (level + 6) == pkmn.level
            levelAdjust = 0.35
            elsif (level + 7) == pkmn.level
            levelAdjust = 0.3
            elsif (level + 8) == pkmn.level
            levelAdjust = 0.25
            else
            levelAdjust = 0.2     #<---- 80% less xp when more than 8 level difference
        end
        elsif level > pkmn.level  #<--- level behind foe
            if (level - 1) == pkmn.level
            levelAdjust = 1.05
            elsif (level - 2) == pkmn.level
            levelAdjust = 1.1
            elsif (level - 3) == pkmn.level
            levelAdjust = 1.15
            elsif (level - 4) == pkmn.level
            levelAdjust = 1.2
            elsif (level - 5) == pkmn.level
            ...
            levelAdjust = 1.6
            elsif (level - 9) == pkmn.level
            levelAdjust = 1.8
            else
            levelAdjust = 2     #<---- xp X2 when more than 10 level difference
            end
        end
        #------------------------------
        exp *= levelAdjust
