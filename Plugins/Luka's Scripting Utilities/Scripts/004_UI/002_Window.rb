...
module LUTS
  ...
  module UI
    # A sprite-based window renderer that creates windowskin bitmaps using 9-slice scaling.
    class Window < ::Sprites::Base
      # Renders a windowskin to a bitmap using 9-slice scaling.
      # @param width [Integer] desired output window width in pixels
      # @param height [Integer] desired output window height in pixels
      # @param path [String] file path to the windowskin source image
      # @param slice [Rect] the slice boundaries defining the center region for 9-slice scaling
      # @return [void]
