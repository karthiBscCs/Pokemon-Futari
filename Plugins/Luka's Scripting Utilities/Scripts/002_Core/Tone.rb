...
#===============================================================================
# Core extensions for the Tone class, adding lookup-table based shading,
# animatable components and whole-tone channel helpers.
class ::Tone
  ...
  class LookupTable
    ...
    # Builds per-channel lookup tables for the given tone.
    # @param tone [Tone] tone to build lookup tables for
    # @return [Tone::LookupTable] new lookup table instance
    def initialize(tone)
      @tone      = tone
    ...
    # @param tone_adjustment [Integer] tone offset for the channel (-255 to 255)
    # @return [Array<Integer>] 256-entry lookup table of adjusted values
  ...
  # Memoized lookup table for this tone.
  # @return [Tone::LookupTable] lookup table built from this tone
  def lookup_table
    @lookup_table ||= LookupTable.new(self)
  end

  # No-op update for animation interface compatibility.
  # @return [void]
  def update; end

  # Average value across the red, green and blue channels.
  ...
  # @param val [Numeric] value to set on red, green and blue
  # @return [Numeric] assigned value
  def all=(val)
    self.red   = val
    self.green = val
    self.blue  = val
